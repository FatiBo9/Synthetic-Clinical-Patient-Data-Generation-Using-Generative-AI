...
PHASES = [
    ("1", "Copula generation",     "Method 1 — Gaussian copula",        COL_ACCENT),
    ("2", "CTGAN generation",      "Method 2 — generative neural network", COL_ACCENT_2),
    ("V", "Validation",            "Rule-based rejection • consistency", "#16a34a"),
    ("3", "Dataset saving",        "CSV file export",                   "#0891b2"),
    ("4", "Statistical analysis",  "Descriptive stats • correlations",  "#7c3aed"),
    ("5", "Visualization",         "Distributions • heatmaps",         "#db2777"),
    ("6", "ML evaluation",         "3 models • cross-validation",       "#ea580c"),
]
...
MODEL_EN = {
    ...
    "random_forest": "Random Forest", "mlp": "Multi-Layer Perceptron",
}

RULE_EN = {
    "bounds": "Physiological limits",
    "R1_quetelet": "R1 — BMI (Quetelet formula)",
    "R2_friedewald": "R2 — Lipid panel (Friedewald)",
    "R3_pulse_pressure": "R3 — Pulse pressure (SBP − DBP)",
    "R4_glucose_hba1c": "R4 — Glucose / HbA1c consistency",
    "class_coherence": "Diagnostic class consistency",
}

VAR_EN = {
    "age": "Age (years)", "height": "Height (cm)", "weight": "Weight (kg)",
    "bmi": "BMI (kg/m²)", "heart_rate": "Heart Rate (bpm)",
    "sbp": "SBP (mmHg)", "dbp": "DBP (mmHg)", "resp_rate": "Respiratory Rate (/min)",
    "temp": "Temperature (°C)", "fasting_glucose": "Fasting Glucose (mg/dL)",
    "hba1c": "HbA1c (%)", "total_chol": "Total Cholesterol (mg/dL)",
    "ldl": "LDL (mg/dL)", "hdl": "HDL (mg/dL)", "triglycerides": "Triglycerides (mg/dL)",
}

CATVAR_EN = {
    "sex": "Sex", "physical_activity": "Physical Activity",
    "smoking": "Smoking", "alcohol": "Alcohol", "diet_quality": "Diet Quality",
}

MODALITY_EN = {
    "Female": "Female", "Male": "Male",
    "High": "High", "Moderate": "Moderate", "Sedentary": "SSedentary",
    "Current": "Current", "Former": "Former", "Never": "Never",
    "Excessive": "Excessive", "None": "None",
    "Average": "Average", "Good": "Good", "Poor": "Poor",
}

FIG_EN = {
    "histograms_copula": "Histograms — Copula",
    "histograms_ctgan": "Histograms — CTGAN",
    "boxplots_copula": "Boxplots — Copula",
    "scatter_copula": "Scatter Plots — Copula",
    "scatter_ctgan": "Scatter Plots — CTGAN",
    "heatmap_copula": "Heatmaps — Copula",
    "heatmap_ctgan": "Heatmaps — CTGAN",
    "heatmap_comparison": "Heatmaps — Comparison",
    "confusion_copula": "Confusion Matrices",
    "metrics_copula": "ML Performance",
    "cv_copula": "5-Fold Cross-Validation",
}
...
class _ImageViewer(ctk.CTkToplevel):
    def __init__(self, master, path, title):
        ...
        tb("Adjust", self._fit, w=84, fg_color=COL_ACCENT,
           hover_color="#0b7d72").pack(side="left", padx=4)
        tb("100 %", self._actual, w=70).pack(side="left", padx=4)
        ctk.CTkLabel(
            bar, text="Mouse wheel: zoom    •    Drag: pan    •    "
            "Double-click: adjust", font=(FONT_FAMILY, 11),
            text_color=COL_TXT_DIM).pack(side="left", padx=14)
        tb("🗂  File", self._open_file, w=92, fg_color=COL_CARD,
    ...
    def _open_file(self):
        try:
            os.startfile(Path(self._path).absolute()) 
        except Exception:
            pass
...
class DemoApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Synthetic Clinical Data Generation — Demo")
    ...
    def _build_sidebar(self):
        ...
        ctk.CTkLabel(head, text="Synthetic Clinical Patient Data",
                     font=(FONT_FAMILY, 18, "bold"), text_color=COL_TXT,
                     justify="left", anchor="w").pack(anchor="w", pady=(2, 0))
        ctk.CTkLabel(head, text="Gaussian copula  •  CTGAN",
        ...
        self.m_entry = self._field(bar, "Patients per class (M)", "1000",
                                   "Total = 6 × M patients")
        self.seed_entry = self._field(bar, "seed", "42",
                                      "Reproductibility")
        self.epochs_entry = self._field(bar, "Epochs CTGAN", "600",
                                        "Higher = Better but Longer")
        ...
        ctk.CTkLabel(out_box, text="Output Folder / Results",
        ...
        self.runall_btn = ctk.CTkButton(
            actions, text="⏩   Run All (1→6)", height=42,
        ...
        self.load_btn = ctk.CTkButton(
            actions, text="📂   Load Existing Results", height=38,
    ...
    def _build_navigator(self):
        ...
        ctk.CTkLabel(nav, text="Data Generation Pipeline Steps",
    ...
    def _update_header_status(self):
        ...
        if idx in VIEW_ONLY:
            ...
            self.h_status.configure(
                text="✓  Validation statistics below"
                if self.done[idx] else sub)
            return
        self.h_run.grid()
        if self.running_idx == idx:
            self.h_status.configure(text="Execution in progress…")
        elif self.done[idx]:
            self.h_status.configure(text="✓  Step completed — results below")
        else:
        ...
        self.h_run.configure(
            ...
            text="▶  Run this step" if not self.done[idx]
            else "↻  Re-run this step")
    ...
    def _gather_params(self):
        try:
            ...
            self.h_status.configure(
                text="⚠ Invalid parameters (M, seed, epochs, batch must be integers).")
            return None
    ...
    def _run_phase(self, idx, _chain=None):
        ...
        if idx != "1" and not self.done["1"]:
            self._select_phase(idx)
            self.h_status.configure(
                text="⚠ Execute step 1 (copula generation) first.")
            return
    ...
    def _render_placeholder(self, idx):
        ...
        msg = ("Please run step 1 first (Gaussian copula generation)." if locked
               else "Click on « Run this step » to execute "
                    "this phase\nand display its results here.")
    ...
    def _render_running(self, idx):
        ...
        ctk.CTkLabel(f, text=f"{title} — in progress…",
                     font=(FONT_FAMILY, 19, "bold"),
        ...
        if idx == "2":
            rows = [
                ("Training epochs", p.get("ctgan_epochs", "—")),
                ("Batch size", p.get("ctgan_batch_size", "—")),
                ("Patients per class", m if m is not None else "—"),
                ("Total to generate", 6 * m if isinstance(m, int) else "—"),
                ("Seed", p.get("seed", "—")),
            ]
        elif idx == "6":
            rows = [
                ("Protocol", "Split 80/20 + CV 5-fold"),
                ("Models", "Logistic Regression • Random Forest • MLP"),
                ("Evaluated patients", 6 * m if isinstance(m, int) else "—"),
                ("Seed", p.get("seed", "—")),
            ]
        else:
            rows = []
        ...
        if idx == "2":
            ...
            self._run_loss_lbl = ctk.CTkLabel(
                live, text="Loss  —", font=(FONT_FAMILY, 11),
                text_color=COL_TXT_DIM)
        ...
        note = ("Training the generative network then validating generation — "
                "this may take several minutes." if idx == "2"
                else "Training and cross-validation of the 3 models — "
                "the MLP may take some time." if idx == "6"
                else "Please wait, the step is currently running.")
    ...
    def _update_ctgan_info(self, info):
        ...
        if (ll is not None and ll.winfo_exists()
            ...
            ll.configure(text=f"Loss  •  Generator {g}  •  Discriminator {d}")
        et = self._run_eta_lbl
        if et is not None and et.winfo_exists():
            parts = []
            if info.get("eta"):
                parts.append(f"remaining ~{info['eta']}")
            if info.get("rate"):
    ...
    def _render_error(self, tb):
        ...
        ctk.CTkLabel(box, text="✕  An error has occurred.", anchor="w",
    ...
    def _render_generation(self, which):
        ...
        if stats:
            ...
            cards = [
                ("Generated Patients", f"{total:,}".replace(",", " "), COL_ACCENT),
                ("Global Rejection Rate", f"{rate:.1%}", COL_RUN),
                ("Total Attempts", f"{attempts:,}".replace(",", " "),
                 COL_ACCENT_2),
                ("Classes", "6", COL_OK),
            ]
            ...
            if byc:
                card = self._panel(scroll, "Rejection Rate by Class", 1)
                for cls, r in byc.items():
                    self._rate_row(card, CLASS_EN.get(cls, cls), r)

        if df is not None:
            p = self._panel(scroll, f"Dataset Preview ({len(df):,} rows × "
                            f"{df.shape[1]} columns)".replace(",", " "), 2)
            self._table(p, df, max_rows=100)
        elif not stats:
            ctk.CTkLabel(scroll, text="No data available.",
    ...
    def _render_datasets(self):
        ...
        pretty = {"dataset_copula": "Method 1 — Gaussian Copula",
                  "dataset_ctgan": "Method 2 — CTGAN"}
        self._ds_map = {}
        ...
        if not csvs:
            ctk.CTkLabel(wrap, text="No CSV file found. Please run step 3.",
    ...
    def _show_dataset(self, label):
        ...
        loading = ctk.CTkLabel(self._ds_table_holder, text="⏳  Loading…",
        ...
        try:
            ...
            loading.destroy()
            ctk.CTkLabel(self._ds_table_holder, text=f"Error : {e}",
    ...
    def _render_validation(self):
        methods = []
        for key, name in (("copula", "Method 1 — Gaussian copula"),
                          ("ctgan", "MMethod 2 — CTGAN")):
        ...
        if not methods:
            ctk.CTkLabel(
                wrap, text="No validation statistics available.\n"
                "Please run step 1 (copula generation).",
                font=(FONT_FAMILY, 13), text_color=COL_TXT_DIM,
        ...
        ctk.CTkLabel(sel, text="Method :", font=(FONT_FAMILY, 13),
                     text_color=COL_TXT).pack(side="left", padx=(0, 8))
    ...
    def _show_validation(self, label):
        ...
        if not stats:
            ctk.CTkLabel(self._val_holder, text="No data available.",
        ...
        self._cards_row(self._val_holder, [
            ("Accepted Patients", f"{acc:,}".replace(",", " "), COL_OK),
            ("Total Attempts", f"{att:,}".replace(",", " "), COL_ACCENT_2),
            ("Rejected Samples", f"{rej:,}".replace(",", " "), COL_RUN),
            ("Global Rejection Rate", f"{rate:.1%}", COL_ERR),
        ], 0)

        r = 1
        if isinstance(avg, (int, float)):
            card = self._panel(self._val_holder,
                               "Generation efficiency", r); r += 1
            self._kv(card, "Average attempts per accepted patient",
                     f"{avg:.2f}")
            ctk.CTkLabel(
                card, text="Each accepted patient is re-sampled until it "
                "passes the entire validation cascade.",
                font=(FONT_FAMILY, 11), text_color=COL_TXT_DIM,
    ...
    def _render_analysis(self):
        ...
        if not ana:
            ctk.CTkLabel(
                wrap, text="No analysis report found.\nPlease run step 4.",
                font=(FONT_FAMILY, 13), text_color=COL_TXT_DIM,
                justify="center").grid(row=0, column=0, pady=30)
            return
        ...
        if isinstance(ana.get("copula"), dict):
            sections.append("Method 1 — Copula")
        if isinstance(ana.get("ctgan"), dict):
            sections.append("Method 2 — CTGAN")
    ...
    def _analysis_method(self, block):
        ...
        if epi:
            ...
            if isinstance(pats, list) and pats:
                ...
                for p in pats:
                    ...
                    rows.append([
                        CLASS_EN.get(p.get("class"), p.get("class")),
                        VAR_EN.get(p.get("variable"), p.get("variable")),
        ...
        if isinstance(cont, dict) and cont:
            card = self._panel(
                holder, "Descriptive Statistics — Continuous Variables", r)
            r += 1
            rows = []
    ...
    def _analysis_comparison(self, mc):
        ...
        if isinstance(div, list) and div:
            card = self._panel(
                holder, "Largest correlation divergences "
                "(Copule vs CTGAN)", r); r += 1
            rows = []
            for x in div:
                if not isinstance(x, dict):
                    continue
                pair = (f"{VAR_EN.get(x.get('var1'), x.get('var1'))}  ×  "
                        f"{VAR_EN.get(x.get('var2'), x.get('var2'))}")
                rows.append([pair, str(x.get("corr_a")), str(x.get("corr_b")),
                             str(x.get("delta"))])
            self._grid_table(card, ["Pair of variables", "Copule", "CTGAN",
                                    "Δ"], rows)
            ctk.CTkLabel(
                card, text="Δ = Absolute difference between Pearson correlations "
                "of both methods.", font=(FONT_FAMILY, 10),
        ...
        if isinstance(cm, dict) and cm:
            card = self._panel(
                holder, "Class means — Copula vs CTGAN", r); r += 1
            self._cmp_means = cm
            self._cmp_class_map = {CLASS_EN.get(c, c): c for c in cm}
    ...
    def _show_cmp_class(self, label):
        ...
        for var, st in data.items():
            ...
            rows.append([VAR_EN.get(var, var), str(a), str(b), delta])
    ...
    def _render_figures(self, patterns):
        ...
        if not pngs:
            ctk.CTkLabel(self.output, text="No figures available. Run this step.",
                         text_color=COL_TXT_DIM).grid(row=0, column=0, pady=30)
            return
        wrap = ctk.CTkFrame(self.output, fg_color="transparent")
        wrap.grid(row=0, column=0, sticky="nsew")
        ...
        ctk.CTkButton(top, text="🔍  Enlarge / Zoom", width=160, height=32,
        ...
        self._pick_fig(pngs[0], FIG_EN.get(pngs[0].stem, pngs[0].stem))
    ...
    def _render_ml(self):
        ...
        if ml:
            ...
            self._cards_row(scroll, [
                ("Evaluated patients", f"{n:,}".replace(",", " ")
                 if isinstance(n, int) else str(n), COL_ACCENT),
                ("Models", "3", COL_ACCENT_2),
                ("Protocol", "Split + CV 5-fold", COL_OK),
            ], r); r += 1
            ...
            if single:
                ...
                for j, h in enumerate(["Model", "Accuracy", "F1 (macro)", "AUC"]):
                ...
                for model, mt in single.items():
                    ctk.CTkLabel(grid, text=MODEL_EN.get(model, model), anchor="w",
                                 font=(FONT_FAMILY, 12), text_color=COL_TXT).grid(
            ...
            if cv:
                ...
                card = self._panel(
                    scroll, f"5-fold cross-validation ({scoring})", r); r += 1
        ...
        if ml_figs:
            card = self._panel(scroll, "Evaluation Figures", r); r += 1
        ...
        if r == 0 and not ml_figs:
            ctk.CTkLabel(scroll, text="No ML results. Run step 6.",
                         text_color=COL_TXT_DIM).grid(row=0, column=0, pady=30)

    def _popup_fig(self, path, name):
        if not _HAS_PIL:
            try:
                os.startfile(Path(path).absolute()) 
            except Exception:
                pass
            return
        try:
