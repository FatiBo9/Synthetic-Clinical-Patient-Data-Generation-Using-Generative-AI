...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Generate analysis plots for a dataset.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--input", "-i", required=True,
                        help="Dataset to visualize.")
    parser.add_argument("--compare", default=None,
                        help="Second dataset for comparison (optional).")
    parser.add_argument("--output", "-o", default="figures",
                        help="Output directory for PNG figures.")
    parser.add_argument("--label", default="dataset",
                        help="Suffix to add to titles and filenames.")
    return parser
...
def main(argv=None) -> int:
    ...
    log.info(f"Loading : {len(df)} patients")

    suffix = f" — {args.label}"
    label = args.label.lower().replace(" ", "_")

    t0 = time.time()
    log.info("Main figures :")
    ...
    _save_fig(plot_correlation_heatmap(corr, title=f"Correlations — {args.label}"),
